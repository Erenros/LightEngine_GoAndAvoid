...
namespace gcle
{
	...
	Shape::Shape(Entity* pOwner) : mp_Owner(pOwner)
	{
	}
	...
	Rectangle::Rectangle(float32 x, float32 y, float32 height, float32 width, Color color, Entity* pOwner) : Shape(pOwner)
	{
		{
	...
	Triangle::Triangle(float32 x1, float32 y1, float32 x2, float32 y2, float32 x3, float32 y3, Color color, Entity* pOwner) : Shape(pOwner)
	{
	...
	Circle::Circle(float32 x, float32 y, float32 radius, int _smoothness, Color color, Entity* pOwner) : Shape(pOwner)
	{
		if (_smoothness < 3) {
			return;
		}
